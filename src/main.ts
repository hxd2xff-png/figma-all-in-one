...
figma.ui.onmessage = async (msg: any) => {
  ...
  if (msg.type === 'font-mixer') {
    ...
    for (const node of texts) { try { const f = node.fontName; if (f === figma.mixed) { const segs = node.getStyledTextSegments(['fontName']); for (const seg of segs) await figma.loadFontAsync(seg.fontName as FontName); } else await figma.loadFontAsync(f as FontName); applied += applyAutoKerning(node).applied; } catch (_) { failed++; } }
